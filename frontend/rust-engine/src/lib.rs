...
//
// Changes vs rev 2:
//   • Added `file_id` to Clip struct to support multi-file timelines
//   • Added `get_active_clips_json` to safely pass active clips to JS
//   • Cleaned up WASM bindings to match JS worker expectations
//
...
pub struct Clip {
    pub id:               u32,
    pub file_id:          u32, // Added to track which file this clip belongs to
    pub track:            u8,
...
impl Clip {
    const EMPTY: Self = Clip {
        id: 0, file_id: 0, track: 0, start_ms: 0, end_ms: 0, source_offset_ms: 0,
    };
}
...
impl Default for ColorGrade {
    fn default() -> Self {
        ColorGrade {
            ...
            highlights: 0.0, shadows: 0.0, vignette: 0.0, grain: 0.0, lut_id: 0,
        }
    }
}
...
impl IklippaEngine {
    ...
    #[wasm_bindgen]
    pub fn frame_ptr(&self) -> u32 { self.pool.ptr() as u32 }
    
    #[wasm_bindgen]
    pub fn frame_len(&self) -> u32 { self.pool.len() as u32 }
    ...
    pub fn add_clip(
        ...
        self.timeline.add_clip(file_id, track as usize, start_ms, end_ms, source_offset_ms)
    }

    /// Returns a JSON string of active clips to avoid complex wasm-bindgen type mapping
    #[wasm_bindgen]
    pub fn get_active_clips_json(&self, playhead_ms: u32) -> String {
        let mut clips = Vec::new();
        for track in 0..MAX_TRACKS {
            if let Some(clip) = self.timeline.active_clip(track, playhead_ms) {
                let source_ms = clip.source_offset_ms + (playhead_ms - clip.start_ms);
                clips.push(format!(
                    r#"{{"file_id":{},"track":{},"source_ms":{}}}"#,
                    clip.file_id, clip.track, source_ms
                ));
            }
        }
        format!("[{}]", clips.join(","))
