// iKlippa Engine — lib.rs
// Rust/WASM core: timeline state, frame memory pool, pixel compositing, colour grading.
//
...
// JS writes decoded pixel data straight into that view, calls process_frame(), Rust
// reads+writes in place, JS reads the same buffer back into ImageData for canvas.
// Net copies per frame: 0 (the WebCodecs copyTo() writes directly into our WASM heap).
...
impl Clip {
    const EMPTY: Self = Clip { id: 0, track: 0, start_ms: 0, end_ms: 0, source_offset_ms: 0 };
}
...
impl Timeline {
    ...
    pub fn add_clip(&mut self, track: usize, start_ms: u32, end_ms: u32, source_offset_ms: u32) -> u32 {
    ...
    pub fn active_clip(&self, track: usize, playhead_ms: u32) -> Option<&Clip> {
        ...
        for i in 0..count {
            ...
            if playhead_ms >= c.start_ms && playhead_ms < c.end_ms {
                return Some(c);
            }
        }
        None
...
impl FramePool {
    ...
    /// Fast XorShift32 pseudo-random number for grain
    #[inline(always)]
...
/// WASM SIMD is enabled via wasm-pack's --target web (uses wasm32 simd128 when available).
/// The scalar fallback is still fast because the loop is branchless and cache-friendly.
fn apply_color_grade(pool: &mut FramePool, grade: &ColorGrade) {
    ...
    // contrast_scale: value of 0.3 → scale=1.6, value=-0.3 → scale=0.4
    let contrast_scale = 1.0 + grade.contrast * 1.5;

    // Temperature shifts: warm = +R −B, cool = −R +B
    let temp_r = 1.0 + grade.temperature * 0.15;
    let temp_b = 1.0 - grade.temperature * 0.15;

    // Highlight/shadow: luminance-dependent lift/roll
    ...
    for i in 0..total_pixels {
        let base = i * 4;
        // Safety: base + 3 < buf.len() because we sized buf to width*height*4
        ...
        // 3. Luminance for luma-dependent operations
        let luma = 0.2126 * r + 0.7152 * g + 0.0722 * b;

        // 4. Highlights/Shadows (smooth roll-off using smoothstep on luma)
        // highlight_mask: 0 at shadows, 1 at highlights
        ...
        // 7. LUT emulation (fast analytic approximations)
        ...
        let r = r + noise;
        let g = g + noise * 0.8; // grain is slightly less green (film characteristic)
        let b = b + noise * 0.9;
...
fn apply_lut(lut_id: u8, r: f32, g: f32, b: f32) -> (f32, f32, f32) {
    match lut_id {
        ...
        // Fuji Superia: warm shadows, slightly desaturated mids, lifted blacks
        1 => {
            let r = r * 1.05 + 0.02;
            let g = g * 0.98 + 0.01;
            let b = b * 0.92 + 0.04;
...
impl IklippaEngine {
    ...
    pub fn new(width: u32, height: u32) -> IklippaEngine {
        // Wire up console_error_panic_hook in dev builds so Rust panics
        // surface as readable errors in the browser console.
    ...
    ///   // Now write directly into `view` — no copy.
    ///   await videoFrame.copyTo(view);  // WebCodecs writes straight into WASM heap
    ///   engine.process_frame();         // Rust processes in place
    ...
    #[wasm_bindgen]
    pub fn add_clip(&mut self, track: u32, start_ms: u32, end_ms: u32, source_offset_ms: u32) -> u32 {
        self.timeline.add_clip(track as usize, start_ms, end_ms, source_offset_ms)
    }

    /// Returns the source-file timestamp (ms) that should be decoded for a given
    /// playhead position, or u32::MAX if no clip is active on track 0 at that time.
    #[wasm_bindgen]
    pub fn source_ms_for_playhead(&self, playhead_ms: u32) -> u32 {
        match self.timeline.active_clip(0, playhead_ms) {
            Some(clip) => {
                let offset_in_clip = playhead_ms - clip.start_ms;
                clip.source_offset_ms + offset_in_clip
            }
            None => u32::MAX,
        }
    }
    ...
    // NOTE: Phase 1 only composites track 0. Phase 2 will add alpha blending
    // for overlays using this same buffer — we write track N into a scratch
    // buffer, then blend it onto the main buffer with premultiplied alpha.
    // The API surface below is designed to accommodate that without breaking changes.
